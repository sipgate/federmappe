...
class StringMapDecoderTest {
    ...
    fun deserializeDataClassWithStackedMapsSucceeds() {
        ...
        val serializer = serializer<TestClass>()
        val data = mapOf<String, Any?>("a" to mapOf("" to mapOf("test2" to "testString2")))

        // Act
        val result = serializer.deserialize(StringMapToObjectDecoder(data))

        // Assert
        assertEquals(mapOf("" to mapOf("test2" to "testString2")), result.a)
        assertIs<TestClass>(result)
    }
    ...
    fun deserializeDataClassWithFilledMapSucceeds() {
        ...
        val serializer = serializer<TestClass>()
        val data =
            mapOf<String, Any?>("a" to mapOf("" to "testString", "test2" to "testString2"))

        // Act
        val result = serializer.deserialize(StringMapToObjectDecoder(data))

        // Assert
        assertEquals(mapOf("" to "testString", "test2" to "testString2"), result.a)
        assertIs<TestClass>(result)
    }
